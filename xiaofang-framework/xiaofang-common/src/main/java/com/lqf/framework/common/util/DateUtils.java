package com.lqf.framework.common.util;
...
public class DateUtils {
    ...
    public static String localDateTime2String(LocalDateTime time) {
        return time.format(DateConstantsEnum.DATE_FORMAT_Y_M_D_H_M_S.getValue());
    }
    ...
    public static String formatRelativeTime(LocalDateTime dateTime) {
        ...
        if (daysDiff < 1) {  // 如果是今天
            ...
            return "昨天 " + dateTime.format(DateConstantsEnum.DATE_FORMAT_H_M.getValue());
        } else if (daysDiff < 7) {  // 如果是最近一周
            return daysDiff + "天前";
        } else if (dateTime.getYear() == now.getYear()) {  // 如果是今年
            return dateTime.format(DateConstantsEnum.DATE_FORMAT_M_D.getValue());
        } else {  // 如果是去年或更早
            return dateTime.format(DateConstantsEnum.DATE_FORMAT_Y_M_D.getValue());
        }
    }
