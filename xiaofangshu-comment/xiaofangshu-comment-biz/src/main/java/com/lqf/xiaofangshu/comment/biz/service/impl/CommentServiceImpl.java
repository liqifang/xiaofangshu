package com.lqf.xiaofangshu.comment.biz.service.impl;
...
import com.lqf.xiaofangshu.comment.biz.domain.dataobject.CommentLikeDO;
import com.lqf.xiaofangshu.comment.biz.domain.mapper.CommentDOMapper;
import com.lqf.xiaofangshu.comment.biz.domain.mapper.CommentLikeDOMapper;
import com.lqf.xiaofangshu.comment.biz.domain.mapper.NoteCountDOMapper;
import com.lqf.xiaofangshu.comment.biz.enums.*;
import com.lqf.xiaofangshu.comment.biz.model.dto.LikeUnlikeCommentMqDTO;
import com.lqf.xiaofangshu.comment.biz.model.dto.PublishCommentMqDTO;
import com.lqf.xiaofangshu.comment.biz.model.vo.*;
import com.lqf.xiaofangshu.comment.biz.retry.SendMqRetryHelper;
import com.lqf.xiaofangshu.comment.biz.rpc.DistributedIdGeneratorRpcService;
import com.lqf.xiaofangshu.comment.biz.rpc.KeyValueRpcService;
import com.lqf.xiaofangshu.comment.biz.rpc.UserRpcService;
import com.lqf.xiaofangshu.comment.biz.service.CommentService;
import com.lqf.xiaofangshu.kv.dto.req.FindCommentContentReqDTO;
...
public class CommentServiceImpl implements CommentService {
    ...
    public PageResponse<FindChildCommentItemRspVO> findChildCommentPageList(FindChildCommentPageListReqVO findChildCommentPageListReqVO) {
        ...
        // 定义每页展示的二级评论数（我观察到，在小红书 APP 中是一次查询 6 条二级评论）；
        long pageSize = 6;
        ...
        Number redisCount = (Number) redisTemplate.opsForHash()
                .get(countCommentKey, RedisKeyConstants.FIELD_COMMENT_TOTAL);
        ...
        if (Boolean.TRUE.equals(hasKey) && offset < 6*10) {
    ...
    public Response<?> likeComment(LikeCommentReqVO likeCommentReqVO) {
        ...
        // 对返回结果进行判断，有 3 种情况：
        // Redis 中布隆过滤器不存在，先写个 TODO, 后续再补充具体逻辑;
        ...
        switch (Objects.requireNonNull(commentLikeLuaResultEnum)) {
            // Redis 中布隆过滤器不存在
            case NOT_EXIST -> {
    ...
    public Response<?> deleteComment(DeleteCommentReqVO deleteCommentReqVO) {
        ...
        if (!Objects.equals(commentDO.getUserId(), currUserId)) {
    ...
    private void checkCommentIsExist(Long commentId) {
        ...
        if (StrUtil.isBlank(localCacheJson)) {
            // 若不存在，则查询 Redis 校验是否存在；
        ...
        for (CommentDO childCommentDO : childCommentDOS) {
            ...
            if (!isContentEmpty) {
                FindCommentContentReqDTO findCommentContentReqDTO = FindCommentContentReqDTO.builder()
                        ...
                        .yearMonth(DateConstantsEnum.DATE_FORMAT_Y_M.getValue().format(childCommentDO.getCreateTime()))
                        .build();
        ...
        allCommentDOS.forEach(commentDO -> {
            ...
            if (!isContentEmpty) {
                FindCommentContentReqDTO findCommentContentReqDTO = FindCommentContentReqDTO.builder()
                        ...
                        .yearMonth(DateConstantsEnum.DATE_FORMAT_Y_M.getValue().format(commentDO.getCreateTime()))
                        .build();
                findCommentContentReqDTOS.add(findCommentContentReqDTO);
            }
